...
- **权限/计划提示桥接** —— Claude terminal prompt 会显示为飞书提示卡；当前用卡片中列出的编号作出选择
...
当前 Claude prompt 的飞书按钮回调尚未接入；看到 `Claude needs input` 卡片时，请直接回复卡片中列出的编号。选项数量不固定，cclark 会从 Claude 当前终端 prompt 中读取全部可见编号。只有 plan mode 中的 `3. Tell Claude what to change` 是两步流程：先回复 `3`，再发送一条反馈文本；cclark 会把第二条文本提交给 Claude 的反馈输入框。
...
| Input card | Claude permission / plan / selection prompt | 显示当前 terminal prompt；回复卡片中列出的编号，plan 选项 `3` 会进入“等待反馈文本”状态 |
