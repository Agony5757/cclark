...
CCLark 是 [unified-icc](https://github.com/Agony5757/unified-icc) 的飞书前端。它作为常驻服务运行，通过 WebSocket 长连接实时接收飞书消息，再通过 unified-icc API Server 的 WebSocket 连接代理到 tmux 上的 interactive coding CLI，并将智能体输出以交互卡片的形式推送回来。
...
CCLark bridges Feishu WebSocket events to the standalone unified-icc WebSocket API server and routes server events back to Feishu cards. It no longer embeds the tmux gateway in the bot process.
...
| **ICC WS 代理** | `icc_ws_gateway.py` | 连接 unified-icc WebSocket API，提供 handlers 兼容的 gateway 方法 |
| **网关集成** | `main.py` | 启动 ICC WS 代理，注入 adapter，注册回调 |
...
CCLark 通过 `~/.unified-icc/config.yaml` 配置（首选）：

```yaml
unified_icc_ws_url: "ws://127.0.0.1:8900/api/v1/ws"
unified_icc_api_key: ""       # 如果 unified-icc 设置了 ICC_API_KEY，这里填同一个值
...
**环境变量回退**（仅单应用、仅开发用）：设置 `FEISHU_APP_ID` / `FEISHU_APP_SECRET`，当 `config.yaml` 不存在时使用。可用 `UNIFIED_ICC_DIR` 覆盖配置目录。

### 启动

```bash
uv run unified-icc server start --port 8900
uv run cclark
```

第一条命令启动 tmux/agent 管理服务，第二条命令启动飞书机器人桥接服务。无需公网 Webhook URL。
